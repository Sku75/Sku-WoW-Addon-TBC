...
## Public API / exports
- `Sku.DeferredData` — registry state: `{registered = {key -> {builderGlobalNames}}, ready = {key -> true}, failed = {key -> true}, buildMs = {key -> ms}, buildSteps = {…}, buildWorkers = {name -> isAliveFn}}` (last two added in #15).
