...
- Purpose: The addon's event broker, used as a **deliberate hybrid** (not the single mandatory bus — see W6-B #11). Modules register callbacks here for a named event (either a real WoW event name or a synthetic `SKU_*` event). On the first callback for a real WoW event, the dispatcher registers that event with itself; a single generated per-event handler then fans the event out to every registered callback. It is a tiny AceAddon (AceConsole/AceEvent) with no features of its own beyond this fan-out. In practice the `SKU_*` events are the genuine cross-module channel, and the raw-WoW-event fan-out is used by the **SkuCore object family** (Core/aqCombat/aq/DialTargeting/turnToUnit/skuFocus) because several SkuCore sub-files must subscribe to the same WoW event, which one AceEvent object cannot. Every other module registers its own WoW events directly via its own AceEvent object — idiomatic Ace3, intended, not a thing to normalize onto the dispatcher.
