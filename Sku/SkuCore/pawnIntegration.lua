...
function PawnIntegration:MenuBuilder()
	...
	if tSettings.pawnIntegrationEnabled == nil then tSettings.pawnIntegrationEnabled = true end
	...
	pcall(function()
		...
		for tStat, tNewValue in pairs(tNewStats or {}) do
			...
			for i, tSection in ipairs(tTextFull) do
				if type(tSection) == "string" and tSection:find(tostring(tLabel), 1, true) then
					local tDecimals = tostring(tStat):find("DAMAGE_PER_SECOND", 1, true) and 1 or 0
					tTextFull[i] = tSection:gsub("([^\r\n]*"..tostring(tLabel):gsub("([^%w])", "%%%1").."[^\r\n]*)", "%1; "..FormatSigned(tDelta, tDecimals), 1)
					break
				end
			end
		end
