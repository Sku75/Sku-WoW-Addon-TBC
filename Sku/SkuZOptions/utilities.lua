...
function SkuTableToString(aTable, aCallback)
	-- Performance: jeder String-Teil wird DIREKT in tChunks gepusht
	-- (Array-Collect statt linear wachsender String). Das vorherige
	-- "tPartString = tPartString .. ..." war O(n²) in der Anzahl der
	-- Konkatenationen — bei einem FullScan mit ~10k Auktionen kostete
	-- das schnell mehrere Sekunden bis Minuten.
	-- Außerdem: kein Throttle in der Coroutine-Pumpe — wir resumen so
	-- oft wie das Frame-Budget zulässt (ein yield pro 2500 Einträge).
	local tChunks = { "{" }
	local co = coroutine.create(function()
		local tLocalCounter = 0
		local function tf(ttable)
			for k, v in pairs(ttable) do
				...
				elseif tv == "boolean" then
					tChunks[#tChunks + 1] = "[" .. k .. "] = " .. tostring(v) .. ","
				elseif tv == "string" then
					tChunks[#tChunks + 1] = "[" .. k .. "] = \"" .. v .. "\","
				else
					tChunks[#tChunks + 1] = "[" .. k .. "] = " .. v .. ","
				end
				tLocalCounter = tLocalCounter + 1
				if tLocalCounter >= 2500 then
					tLocalCounter = 0
					coroutine.yield()
				end
			end
	...
	tSkuCoroutineControlFrame:SetScript("OnUpdate", function(self, time)
		if coroutine.status(co) == "suspended" then
			...
			if tCoCompleted == false then
				...
				aCallback("return " .. table.concat(tChunks))
			end
		end
	end)
end
