...
IsCharSelectionScreen()
{
	...
	tAddonsMatches := (IsColorRange(tRGBColorAddons.r, gGameUiColors.GenericRedButton.r) = true and IsColorRange(tRGBColorAddons.g, gGameUiColors.GenericRedButton.g) = true and IsColorRange(tRGBColorAddons.b, gGameUiColors.GenericRedButton.b) = true)
	tLogoMatches := (IsColorRange(tRGBColorLogo.r, gGameUiColors.CharSelectionScreenLogo.r) = true and IsColorRange(tRGBColorLogo.g, gGameUiColors.CharSelectionScreenLogo.g) = true and IsColorRange(tRGBColorLogo.b, gGameUiColors.CharSelectionScreenLogo.b) = true)

	; BurningCrusade Anniversary has no yellow logo at (38,72) on CharSelect; rely on Addons-only.
	if ((gHasSetupGametype = "BurningCrusade" and tAddonsMatches) or (tLogoMatches and tAddonsMatches))
	{
		rReturnValue := true
	}
...
IsCharCreationScreen()
{
	...
	tBackdropMatches := (IsColorRange(tRGBColorRCBackdrop.r, gGameUiColors.CharCreationBackdrop.r) = true and IsColorRange(tRGBColorRCBackdrop.g, gGameUiColors.CharCreationBackdrop.g) = true and IsColorRange(tRGBColorRCBackdrop.b, gGameUiColors.CharCreationBackdrop.b) = true)
	tLogoMatches := (IsColorRange(tRGBColorLogo.r, gGameUiColors.CharCreationLogo.r) = true and IsColorRange(tRGBColorLogo.g, gGameUiColors.CharCreationLogo.g) = true and IsColorRange(tRGBColorLogo.b, gGameUiColors.CharCreationLogo.b) = true)

	; BC Anniversary: logo shifts by ~10 UI-px between frames (idle animation). Scan several candidate positions and accept any yellow hit.
	if (gHasSetupGametype = "BurningCrusade")
	{
		if (tBackdropMatches and IsBcCharCreationLogoYellow())
		{
			rReturnValue := true
		}
	}
	else if (tLogoMatches and tBackdropMatches)
	{
		rReturnValue := true
	}
...
;------------------------------------------------------------------------------------------
IsBcCharCreationLogoYellow()
{
	; Candidate positions derived from two logo scans taken seconds apart (y-range 66..80, x-range 108..140).
	; Yellow test: R>180, G>200, B<30 — tolerant to minor AA / animation drift.
	tCandidates := [[108,68],[140,74],[110,78],[112,72],[136,72],[140,78],[112,80],[110,70],[136,68]]
	Loop % tCandidates.MaxIndex()
	{
		tPos := tCandidates[A_Index]
		tC := GetColorAtUiPos(tPos[1], tPos[2])
		if (tC.r > 180 and tC.g > 200 and tC.b < 30)
			return true
	}
	return false
}
...
Is11Popup()
{
	...
	; BC Anniversary name-not-available popup: OK button sits lower than in Classic/Cata/Retail.
	; Measured from Fehler Erstellung.png: red spans UI y ~400..405 at UI x=9915 (button left edge).
	tProbeY := 386
	if (gHasSetupGametype = "BurningCrusade")
		tProbeY := 400
	tRGBColor := GetColorAtUiPos(9915, tProbeY)
